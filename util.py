...
def read_license_plate(plate_image):
    """
    Reads the license plate text from a preprocessed plate image.
    
    Args:
        plate_image: Preprocessed binary image of license plate
        
    Returns:
        tuple: (extracted text, confidence score)
    """
    # No need to import pytesseract here since it's already imported at the top
    
    # Ensure image is properly sized for OCR
    plate_image = cv2.resize(plate_image, None, fx=2, fy=2, interpolation=cv2.INTER_CUBIC)
    
    # Apply additional preprocessing
    # Try adaptive thresholding
    plate_image = cv2.adaptiveThreshold(plate_image, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, 
